...
class FasterAugmenter:
    ...
    def augment(self, text):
        ...
        for _ in range(self.transformations_per_example):
            ...
            transformed_modify_indices = [list(text.attack_attrs["modified_indices"])[0] for text in transformed_texts]
            transformed_modify_items = defaultdict(list)
            for modify_index, text in zip(transformed_modify_indices, transformed_texts):
                transformed_modify_items[modify_index].append(text)
            
            if len(transformed_modify_items.keys()) <= num_words_to_swap:
                chosen_indices = list(transformed_modify_items.keys())
            else:
                chosen_indices = np.random.choice(list(transformed_modify_items.keys()), size=(num_words_to_swap,), replace=False).tolist()
            
            chosen_transformed_texts = [random.choice(transformed_modify_items[index]) for index in chosen_indices]
            for text, index in zip(chosen_transformed_texts, chosen_indices):
                current_text = current_text.replace_word_at_index(index, text.words[index])

            all_transformed_texts.add(current_text)
        
        if all_transformed_texts:
            return sorted([at.printable_text() for at in all_transformed_texts])
        else:
            return [original_text.printable_text() for _ in range(self.transformations_per_example)]
